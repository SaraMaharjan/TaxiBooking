...
/**
 * DriverMyProfilePage — Driver profile editor.
 * Extends BaseAppPage for sidebar behaviour.
 * Uses yellow avatar circle to visually distinguish Driver from Passenger.
 */
public class DriverMyProfilePage extends BaseAppPage {
    ...
    public DriverMyProfilePage() {
        super();        // buildContent() runs here, fields assigned inside it
        loadUserData(); // safe to call now
    }
    ...
    private VBox buildAvatarCol() {
        ...
        // Yellow background — visually distinguishes Driver profile
        javafx.scene.shape.Circle yellowBg = new javafx.scene.shape.Circle(67.5);
        yellowBg.setFill(javafx.scene.paint.Color.web("#F2C94C"));
        yellowBg.setEffect(new javafx.scene.effect.DropShadow(
            10, 0, 2, javafx.scene.paint.Color.rgb(0, 0, 0, 0.10)));
        frame.getChildren().add(yellowBg);
        ...
        try {
            ...
            Label fb = new Label("👤"); fb.setStyle("-fx-font-size: 52px;");
            frame.getChildren().add(fb);
        }
 
        Label roleLabel = new Label("Driver");
